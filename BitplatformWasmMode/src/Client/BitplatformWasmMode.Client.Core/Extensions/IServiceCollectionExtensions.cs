...
using BitplatformWasmMode.Client.Core.Components;
using BitplatformWasmMode.Client.Core.Services.HttpMessageHandlers;
    ...
    public static IServiceCollection AddClientCoreProjectServices(this IServiceCollection services, IConfiguration configuration)
    {
        ...
        services.AddBesqlDbContextFactory<OfflineDbContext>(options =>
        {
            ...
            var dirPath = isRunningInsideDocker ? "/container_volume"
                                                : AppPlatform.IsBlazorHybridOrBrowser ? Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "54B641E5-96F0-430B-B79E-499FD77BBC20")
